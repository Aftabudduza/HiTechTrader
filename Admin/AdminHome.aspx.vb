...
Partial Class Admin_AdminHome
    ...
    Public Sub showTotalProduct()
        Try
            ...
            strQuery = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and p.IsDoNotRelease = 1 and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL)   "
            lblDoNotRelease.Text = ShowTotal(strQuery)

            strQuery = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and p.IsFeaturedItem = 1  and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL)  "
            lblFeatured.Text = ShowTotal(strQuery)

            strQuery = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and p.IsSold = 1 and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND (p.IsLabX = 0 OR p.IslabX IS NULL) and p.DateSold BETWEEN DATEADD(dd, -30, GETDATE()) AND GETDATE() "
            lblSold.Text = ShowTotal(strQuery)

            strQuery = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and p.IsDeleteItem = 1 AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL)  and p.DateDeleted BETWEEN DATEADD(dd, -30, GETDATE()) AND GETDATE() "
            lblDelete.Text = ShowTotal(strQuery)

            strQuery = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and p.IsSold = 1  and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND (p.IsLabX = 0 OR p.IslabX IS NULL) and (p.DateSold NOT  BETWEEN DATEADD(dd, -30, GETDATE()) AND GETDATE() OR p.DateSold IS NULL) "
            lblArchiveSold.Text = ShowTotal(strQuery)

            strQuery = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0  and (p.IsDeleteItem = 1  or p.IsDeletePermanently = 1) AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL) and (p.DateDeleted NOT  BETWEEN DATEADD(dd, -30, GETDATE()) AND GETDATE() OR p.DateDeleted IS NULL) "
