...
Partial Class Admin_EquipmentList
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
    End Sub
    Private Sub fill_MyEquipmentList()
        Dim htmlStr As String = ""
        Dim query As String = ""
        Try
            query = " SELECT DISTINCT c.Id, MAX(c.CategoryName) NAME, COUNT(pccr.ProductId) total   FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId  AND c.IsMainorLabX = 0  "
            ...
            If Not ds Is Nothing And ds.Tables(0).Rows.Count > 0 Then
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    For Each dr As DataRow In ds.Tables(0).Rows
                        ...
                        Else
                            If j > m Then
                                tbl.Append("<td></td><td class='ListCategoryColumn'><div><h3 class='MainCategory'><a style='color:#0000cc;' href='../Admin/ProductListing.aspx?sc_state=0&sc_page=0&sc_cat=" & dr("Id").ToString() & "' class='h3a'>" & dr("NAME") & "&nbsp;&nbsp;" & "(" & dr("total") & ")" & "</a></h3><ul class='ListCategory'>")
                                j = 0
                            Else
                                tbl.Append("<div><h3 class='MainCategory'><a style='color:#0000cc;' href='../Admin/ProductListing.aspx?sc_state=0&sc_page=0&sc_cat=" & dr("Id").ToString() & "' class='h3a'>" & dr("NAME") & "&nbsp;&nbsp;" & "(" & dr("total") & ")" & "</a></h3><ul class='ListCategory'>")
                            End If
                        End If
                ...
                End If
                tbl.Append("</table>")
                EquipmentList.InnerHtml = tbl.ToString()
            Else
    ...
    Private Sub fill_MyCategories()
        ...
        If rdoEquipmentList.SelectedIndex <> -1 Then
            ...
            End If
            If rdoEquipmentList.Items(3).Selected = True Then
                sWhere = "AND p.IsJustOfftheTruck = 1 "
            End If
            If rdoEquipmentList.Items(4).Selected = True Then
                sWhere = "AND p.IsLabX = 1 "
            End If
            If rdoEquipmentList.Items(5).Selected = True Then
                sWhere = "AND p.IsDoNotRelease = 1 "
            End If
            If rdoEquipmentList.Items(6).Selected = True Then
                sWhere = "AND p.IsConsignmentItem = 1 "
            End If
            If rdoEquipmentList.Items(7).Selected = True Then
                sWhere = "AND p.IsSold = 1 "
            End If
            If rdoEquipmentList.Items(8).Selected = True Then
                sWhere = "AND p.IsDeleteItem = 1 "
            End If
            If rdoEquipmentList.Items(9).Selected = True Then
                sWhere = "AND p.IsDeletePermanently = 1 "
            End If
        End If
        Try
            query = " SELECT DISTINCT  c4.Id, max(c4.CategoryName) Name, Total =ISNULL((SELECT COUNT(p.Id) Total FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId  AND c.IsMainorLabX = 0 " & sWhere.Trim & "  and (c.CategoryParentId = c4.Id OR c.Id = c4.Id OR c.CategoryParentId IN (SELECT c2.Id  FROM Category c2 WHERE c2.CategoryParentId IN (SELECT c3.CategoryParentId  FROM Category c3 WHERE c3.CategoryParentId =c4.Id)))),0)  FROM Category c4 WHERE c4.CategoryParentId = 0 AND c4.IsMainorLabX = 0  GROUP BY c4.Id HAVING ISNULL((SELECT COUNT(p.Id) Total FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId  AND c.IsMainorLabX = 0 " & sWhere.Trim & " and (c.CategoryParentId = c4.Id OR c.Id = c4.Id OR c.CategoryParentId IN (SELECT c2.Id  FROM Category c2 WHERE c2.CategoryParentId IN (SELECT c3.CategoryParentId  FROM Category c3 WHERE c3.CategoryParentId =c4.Id)))),0) > 0  ORDER BY max(c4.CategoryName) ASC "
            ...
            If Not ds Is Nothing And ds.Tables(0).Rows.Count > 0 Then
                ...
                tbl.Append("</table>")
            End If
            htmlStr = tbl.ToString()

            EquipmentList.InnerHtml = htmlStr

        Catch ex As Exception
        End Try
    End Sub
    Protected Sub rdoEquipmentList_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rdoEquipmentList.SelectedIndexChanged
        Try
            fill_MyCategories()
        Catch ex As Exception
