...
Partial Class Admin_ProductListing
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Session("Id") Is Nothing Then
            ...
            End Try
            Try
                sParent = Request.QueryString("parent").ToString()
            Catch ex As Exception
                sParent = ""
            End Try
            If Not Page.IsPostBack Then
                ...
                If ProductCategoryId > 0 Then
                    ...
                    End If
                    Try
                        spanManual.InnerHtml = ShowManualsByCategoryId(ProductCategoryId)
                    Catch ex As Exception
                ...
                Else
                    ...
                    ElseIf ProductStateId = 9 Then
                        ...
                        Session("pagerSQL") = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and p.IsSold = 1  and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND (p.IsLabX = 0 OR p.IslabX IS NULL) and (p.DateSold NOT  BETWEEN DATEADD(dd, -30, GETDATE()) AND GETDATE() OR p.DateSold IS NULL) " & strWhere.Trim
                        If Not Session("strOrder") Is Nothing Then
                            sSQl = Session("pagerSQL") & Session("strOrder").ToString()
                            SqlDataSource1.SelectCommand = sSQl
                        Else
                    ...
                    ElseIf ProductStateId = 10 Then
                        ...
                        Session("pagerSQL") = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0  and (p.IsDeleteItem = 1  or p.IsDeletePermanently = 1) AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL) and (p.DateDeleted NOT  BETWEEN DATEADD(dd, -30, GETDATE()) AND GETDATE() OR p.DateDeleted IS NULL) " & strWhere.Trim
                        If Not Session("strOrder") Is Nothing Then
                            sSQl = Session("pagerSQL") & Session("strOrder").ToString()
                            SqlDataSource1.SelectCommand = sSQl
                        Else
                    ...
                    If sParent.Length > 0 Then
                        lblCategory.InnerHtml = ""
                        Session("pagerSQL") = " SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.IsSold,0) Sold, isnull(p.Price,0) Price, isnull(p.LowestPrice,0) LowestPrice, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsLabX,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId and p.CreatorID = " & CInt(Session("Id").ToString()) & "  AND c.IsMainorLabX = 0 and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL) AND p.BarcodeParent = '" & sParent.Trim & "' UNION SELECT p.Id, p.ItemNumber, p.ProductName, p.Description, isnull(p.Price,0) Price, isnull(p.Location,'') Location, isnull(p.Barcode,'') Barcode, isnull(p.BarcodeParent,'') BarcodeParent, isnull(p.IsNotOnWeb,0) IsNotOnWeb, isnull(p.IsConsignmentItem,0) IsConsignmentItem, isnull(p.VideoURL,'') VideoURL, isnull(C.Id,0) CatId, isnull(c.CategoryName,'') CategoryName, isnull(c.CategoryParentId,'') ParentCategory, Parent = isnull((SELECT TOP 1 c2.CategoryName FROM Category c2 WHERE c2.Id = c.CategoryParentId),'') FROM Product p, Category c, ProductCategoryCrossRef pccr WHERE c.Id = pccr.ProductCategoryId AND p.Id = pccr.ProductId  AND c.IsMainorLabX = 0 and (p.IsDeleteItem <> 1  or p.IsDeletePermanently <> 1) AND p.IsSold <> 1 AND (p.IsLabX = 0 OR p.IslabX IS NULL) AND p.Barcode = '" & sParent.Trim & "'"
                        If Not Session("strOrder") Is Nothing Then
                            sSQl = Session("pagerSQL") & Session("strOrder").ToString()
                            SqlDataSource1.SelectCommand = sSQl
                        Else
                    ...
                    If sSearch.Length > 0 Then
                        ...
                        lblSearch.InnerHtml = "<strong>Search term: </strong> " & sSearch.Trim
        ...
        Try
            If sPTitle.Length > 0 Then
                If sTitle.Length > 0 Then
                    str = "Category: <a title='" & sPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & pId & "'>" & sPTitle & "</a> "
                    str &= "&nbsp;&nbsp;>>&nbsp;&nbsp;<a title='" & sTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & Id & "'>" & sTitle & "</a> "
                Else
                    str = "Category: <a title='" & sPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & pId & "'>" & sPTitle & "</a> "
                End If
            End If
    ...
    Public Function ShowCategoryName(ByVal Id As Integer) As String
        ...
        Try
            ...
            If sGPTitle.Length > 0 Then
                If sPTitle.Length > 0 Then
                    If sTitle.Length > 0 Then
                        str = "Category: <a title='" & sGPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("GrandParentId").ToString() & "'>" & sGPTitle & "</a> "
                        str &= "&nbsp;&nbsp;>>&nbsp;&nbsp;<a title='" & sPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("ParentId").ToString() & "'>" & sPTitle & "</a> "
                        str &= "&nbsp;&nbsp;>>&nbsp;&nbsp;<a title='" & sTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("Id").ToString() & "'>" & sTitle & "</a> "
                    Else
                        str = "Category: <a title='" & sGPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("GrandParentId").ToString() & "'>" & sGPTitle & "</a> "
                        str &= "&nbsp;&nbsp;>>&nbsp;&nbsp;<a title='" & sPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("ParentId").ToString() & "'>" & sPTitle & "</a> "
                    End If
            ...
            Else
                If sPTitle.Length > 0 Then
                    If sTitle.Length > 0 Then
                        str = "Category: <a title='" & sPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("ParentId").ToString() & "'>" & sPTitle & "</a> "
                        str &= "&nbsp;&nbsp;>>&nbsp;&nbsp;<a title='" & sTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("Id").ToString() & "'>" & sTitle & "</a> "
                    Else
                        str = "Category: <a title='" & sPTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("ParentId").ToString() & "'>" & sPTitle & "</a> "
                    End If
                Else
                    str = "Category: <a title='" & sTitle & "' href='../Admin/ProductListing.aspx?sc_state=1&sc_id=1&sc_cat=" & UserDS.Tables(0).Rows(0)("Id").ToString() & "'>" & sTitle & "</a> "
                End If
            End If
    ...
    Public Function ShowVideo(ByVal Id As Integer, ByVal sTitle As String, Optional ByVal sVideoName As String = "") As String
        ...
        Try
            If Not sVideoName Is Nothing Then
                If sVideoName.Length > 0 Then
                    strVideo = "<div style='vertical-align: middle; width:80px; float:left; margin:10px;'><a title='Video for: " & sTitle.Trim & "' href='" & sVideoName.Trim & "' target='_blank'><img width='20' height='20' border='0' align='left' style='padding-right: 0.2em' alt='See a Video of the Item' src='../ProductImages/Large/videoicon.gif'>See Video</a></div>"
                End If
            End If
        ...
        Return ""
    End Function
    Public Function ShowManualsByCategoryId(ByVal nCatId As Integer) As String
        Dim UserDS As New DataSet
        Dim sStrNew As String = ""
        Dim sTitle As String = String.Empty

        Try
            sStrNew = " SELECT p.*  FROM ManualProduct  p, Category c WHERE p.Category = c.Id AND   c.IsMainorLabX = 0 AND p.CreatorID =  " & CInt(Session("Id").ToString()) & "  and (p.Category = " & nCatId & "  or p.ParentCategory = " & nCatId & " )"
            UserDS = SQLData.generic_select(sStrNew, strConnection)

            If Not UserDS Is Nothing Then
                If UserDS.Tables(0).Rows.Count > 0 Then
                    sTitle = "<a href='../Admin/ManualProductListing.aspx?sc_state=1&amp;sc_id=1&amp;sc_cat=" & nCatId & "' >See Manuals</a>"
                End If
            End If
        Catch ex As Exception
            sTitle = ""
