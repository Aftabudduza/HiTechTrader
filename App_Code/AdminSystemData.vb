...
#Region " Constructors "
    ...
    Public Sub New()
        ...
        ColInfo.Add(New ColumnInfo("Type", "Integer", 11, True, True))
        ColInfo.Sort()
    End Sub
...
#Region " Methods "
    ''' <summary>Clear the local column variables associated with a AdminSystemData table record</summary>
    Public Sub Blank()
        Id = 0
        Name = ""
        Type = 0
    ...
    ''' <summary>Get rows from the AdminSystemData table</summary>
    ''' <param name="ColumnList">A list of column names in the AdminSystemData table separated by commas</param>
    ...
    ''' <returns>A DataTable from the AdminSystemData table</returns>
    ...
    Public Function GetRows(ByRef ColumnList As String, ByRef WhereClause As String, ByRef OrderBy As String) As DataTable
        ...
        Try
            ...
            cmd = New SqlCommand("SELECT " & columnlist & " FROM ""AdminSystemData"" WHERE " & whereClause & ob, conn)
    ...
    ''' <summary>Get a rows from the AdminSystemData table</summary>
    ''' <param name="ColumnList">An array of column names in the AdminSystemData</param>
    ''' <param name="WhereClause">An SQL WHERE clause for a SELECT statement that may return multiple rows</param>
    ''' <param name="OrderBy">An SQL ORDER BY clause for a SELECT statement</param>
    ''' <returns>A DataTable from the AdminSystemData table</returns>
    ''' <remarks>Use care with web forms since the resulting SELECT statement is not parameterized</remarks>
    ...
    ''' <summary>Get a rows from the AdminSystemData table</summary>
    ''' <param name="ColumnList">An ArrayList of Strings representing column names in the AdminSystemData</param>
    ''' <param name="WhereClause">An SQL WHERE clause for a SELECT statement that may return multiple rows</param>
    ''' <param name="OrderBy">An SQL ORDER BY clause for a SELECT statement</param>
    ''' <returns>A DataTable from the AdminSystemData table</returns>
    ''' <remarks>Use care with web forms since the resulting SELECT statement is not parameterized</remarks>
    ...
    Public Function Delete(ByVal Id As Integer) As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("DELETE FROM ""AdminSystemData"" WHERE Id = @Id", Conn)
    ...
    Public Function Delete() As Boolean
        ...
        Return Delete(Id)
    End Function
    ''' <summary>Insert a record in the AdminSystemData table from the data stored in the local variables</summary>
    ''' <returns>'True', if successful</returns>
    Public Function Insert() As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("INSERT INTO ""AdminSystemData"" (Name, Type) VALUES (@Name, @Type)", conn)
            ...
            cmd = New SqlCommand("SELECT TOP 1 Id FROM ""AdminSystemData"" WHERE Name = @Name AND Type = @Type ORDER BY Id DESC", conn)
            cmd.Parameters.AddWithValue("@Name", Name)
            cmd.Parameters.AddWithValue("@Type", Type)
            da = New SqlDataAdapter(cmd)
            da.Fill(dt)
        ...
        Return True
    End Function
    Public Function InsertIntoSystemData() As Integer
        Dim cmd As SqlCommand
        ...
        Return checkInsert
    End Function
    ''' <summary>Insert a record in the AdminSystemData table from a DataRow</summary>
    ''' <param name="Row">The DataRow to be inserted in the AdminSystemData table</param>
    ''' <returns>'True', if successful</returns>
    ''' <remarks>The DataRow must contain all primary key columns</remarks>
    Public Function Insert(ByRef Row As DataRow) As Boolean
        ...
        Try
            ...
            For Each col in Row.Table.Columns
                ...
                End Select
            Next
            cmd = New SqlCommand("INSERT INTO ""AdminSystemData"" (" & setList & ") VALUES (" & valList & ")", conn)
    ...
    Public Function Update() As Boolean
        Try
            Connect()
            Dim cmd As SqlCommand
            cmd = New SqlCommand("UPDATE ""AdminSystemData"" SET Name = @Name ,Type = @Type  WHERE Id = @Id", Conn)
            cmd.Parameters.AddWithValue("@Id", Id)
            cmd.Parameters.AddWithValue("@Name", Name)
            cmd.Parameters.AddWithValue("@Type", Type)
            cmd.ExecuteScalar()
    ...
    Public Function Update(ByRef Row As DataRow) As Boolean
        ...
        Try
            ...
            Next
            cmd = New SqlCommand("UPDATE ""AdminSystemData"" SET " & setList & " WHERE Id = @Id", Conn)
            REM Create the parameters
            For Each col In Row.Table.Columns
                cmd.Parameters.AddWithValue("@" & col.ColumnName, Row(col.ColumnName))
            Next
    ...
    End Function
    ''' <summary></summary>
    ''' <returns>The column names from AdminSystemData table separated by commas</returns>
