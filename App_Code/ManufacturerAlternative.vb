...
Public Class ManufacturerAlternative
...
#Region " Methods "
    ''' <summary>Clear the local column variables associated with a ManufacturerAlternative table record</summary>
    Public Sub Blank()
        Id = 0
        ManufacturerId = 0
        Alternative = ""
    ...
    ''' <summary>Get rows from the ManufacturerAlternative table</summary>
    ''' <param name="ColumnList">A list of column names in the ManufacturerAlternative table separated by commas</param>
    ...
    ''' <returns>A DataTable from the ManufacturerAlternative table</returns>
    ...
    Public Function GetRows(ByRef ColumnList As String, ByRef WhereClause As String, ByRef OrderBy As String) As DataTable
        ...
        Try
            ...
            cmd = New SqlCommand("SELECT " & columnlist & " FROM ""ManufacturerAlternative"" WHERE " & whereClause & ob, conn)
    ...
    ''' <summary>Get a rows from the ManufacturerAlternative table</summary>
    ''' <param name="ColumnList">An array of column names in the ManufacturerAlternative</param>
    ''' <param name="WhereClause">An SQL WHERE clause for a SELECT statement that may return multiple rows</param>
    ''' <param name="OrderBy">An SQL ORDER BY clause for a SELECT statement</param>
    ''' <returns>A DataTable from the ManufacturerAlternative table</returns>
    ''' <remarks>Use care with web forms since the resulting SELECT statement is not parameterized</remarks>
    ...
    ''' <summary>Get a rows from the ManufacturerAlternative table</summary>
    ''' <param name="ColumnList">An ArrayList of Strings representing column names in the ManufacturerAlternative</param>
    ''' <param name="WhereClause">An SQL WHERE clause for a SELECT statement that may return multiple rows</param>
    ''' <param name="OrderBy">An SQL ORDER BY clause for a SELECT statement</param>
    ''' <returns>A DataTable from the ManufacturerAlternative table</returns>
    ''' <remarks>Use care with web forms since the resulting SELECT statement is not parameterized</remarks>
    ...
    Public Function Delete(ByVal Id As Integer) As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("DELETE FROM ""ManufacturerAlternative"" WHERE Id = @Id", Conn)
    ...
    Public Function Insert() As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("INSERT INTO ""ManufacturerAlternative"" (ManufacturerId, Alternative) VALUES (@ManufacturerId, @Alternative)", conn)
            cmd.Parameters.AddWithValue("@ManufacturerId", ManufacturerId)
            cmd.Parameters.AddWithValue("@Alternative", Alternative)
            cmd.ExecuteScalar()
            cmd.Dispose()
            ...
            cmd = New SqlCommand("SELECT TOP 1 Id FROM ""ManufacturerAlternative"" WHERE ManufacturerId = @ManufacturerId AND Alternative = @Alternative ORDER BY Id DESC", conn)
            cmd.Parameters.AddWithValue("@ManufacturerId", ManufacturerId)
            cmd.Parameters.AddWithValue("@Alternative", Alternative)
            da = New SqlDataAdapter(cmd)
            da.Fill(dt)
        ...
        Return True
    End Function
    Public Function InsertIntoManufacturerAlt() As Integer
        Dim cmd As SqlCommand
        ...
        Return checkInsert
    End Function
    ''' <summary>Insert a record in the ManufacturerAlternative table from a DataRow</summary>
    ''' <param name="Row">The DataRow to be inserted in the ManufacturerAlternative table</param>
    ''' <returns>'True', if successful</returns>
    ''' <remarks>The DataRow must contain all primary key columns</remarks>
    Public Function Insert(ByRef Row As DataRow) As Boolean
        ...
        Try
            ...
            For Each col in Row.Table.Columns
                ...
                End Select
            Next
            cmd = New SqlCommand("INSERT INTO ""ManufacturerAlternative"" (" & setList & ") VALUES (" & valList & ")", conn)
    ...
    Public Function Update() As Boolean
        Try
            Connect()
            Dim cmd As SqlCommand
            cmd = New SqlCommand("UPDATE ""ManufacturerAlternative"" SET ManufacturerId = @ManufacturerId , Alternative = @Alternative   WHERE Id = @Id", Conn)
            cmd.Parameters.AddWithValue("@Id", Id)
            cmd.Parameters.AddWithValue("@ManufacturerId", ManufacturerId)
            cmd.Parameters.AddWithValue("@Alternative", Alternative)
            cmd.ExecuteScalar()
        ...
        Return True
    End Function
    Public Function UpdateManufacturerAltStatus() As Boolean
        Try
            Connect()
            Dim cmd As SqlCommand
            cmd = New SqlCommand("UPDATE ""ManufacturerAlternative"" SET Alternative = @Alternative WHERE Id = @Id", Conn)
            cmd.Parameters.AddWithValue("@Id", Id)
            cmd.Parameters.AddWithValue("@Alternative", Alternative)
            cmd.ExecuteScalar()
            cmd.Dispose()
    ...
    Public Function Update(ByRef Row As DataRow) As Boolean
        ...
        Try
            ...
            Next
            cmd = New SqlCommand("UPDATE ""ManufacturerAlternative"" SET " & setList & " WHERE Id = @Id", Conn)
            REM Create the parameters
            For Each col In Row.Table.Columns
                cmd.Parameters.AddWithValue("@" & col.ColumnName, Row(col.ColumnName))
            Next
    ...
    End Function
    ''' <summary></summary>
    ''' <returns>The column names from ManufacturerAlternative table separated by commas</returns>
