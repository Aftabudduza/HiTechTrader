...
Public Class ProductImageCrossRef
...
#Region " Methods "
    ''' <summary>Clear the local column variables associated with a ProductImageCrossRef table record</summary>
    Public Sub Blank()
        Id = 0
        ProductId = 0
        ImageUrl = ""
    ...
    ''' <summary>Get rows from the ProductImageCrossRef table</summary>
    ''' <param name="ColumnList">A list of column names in the ProductImageCrossRef table separated by commas</param>
    ...
    ''' <returns>A DataTable from the ProductImageCrossRef table</returns>
    ...
    Public Function GetRows(ByRef ColumnList As String, ByRef WhereClause As String, ByRef OrderBy As String) As DataTable
        ...
        Try
            ...
            cmd = New SqlCommand("SELECT " & columnlist & " FROM ""ProductImageCrossRef"" WHERE " & whereClause & ob, conn)
    ...
    ''' <summary>Get a rows from the ProductImageCrossRef table</summary>
    ''' <param name="ColumnList">An array of column names in the ProductImageCrossRef</param>
    ''' <param name="WhereClause">An SQL WHERE clause for a SELECT statement that may return multiple rows</param>
    ''' <param name="OrderBy">An SQL ORDER BY clause for a SELECT statement</param>
    ''' <returns>A DataTable from the ProductImageCrossRef table</returns>
    ''' <remarks>Use care with web forms since the resulting SELECT statement is not parameterized</remarks>
    ...
    ''' <summary>Get a rows from the ProductImageCrossRef table</summary>
    ''' <param name="ColumnList">An ArrayList of Strings representing column names in the ProductImageCrossRef</param>
    ''' <param name="WhereClause">An SQL WHERE clause for a SELECT statement that may return multiple rows</param>
    ''' <param name="OrderBy">An SQL ORDER BY clause for a SELECT statement</param>
    ''' <returns>A DataTable from the ProductImageCrossRef table</returns>
    ''' <remarks>Use care with web forms since the resulting SELECT statement is not parameterized</remarks>
    ...
    Public Function Delete(ByVal Id As Integer) As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("DELETE FROM ""ProductImageCrossRef"" WHERE Id = @Id", Conn)
    ...
    Public Function DeleteByImageAndProductId(ByVal ProductId As Integer, ByVal Image As String) As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("DELETE FROM ""ProductImageCrossRef"" WHERE ProductId = @ProductId and ImageUrl = @ImageUrl", Conn)
    ...
    ''' <summary>Insert a record in the ProductImageCrossRef table from the data stored in the local variables</summary>
    ''' <returns>'True', if successful</returns>
    Public Function Insert() As Boolean
        ...
        Try
            Connect()
            cmd = New SqlCommand("INSERT INTO ""ProductImageCrossRef"" (ProductId, ImageUrl) VALUES (@ProductId, @ImageUrl)", conn)
            cmd.Parameters.AddWithValue("@ProductId", ProductId)
            cmd.Parameters.AddWithValue("@ImageUrl", ImageUrl)
            cmd.ExecuteScalar()
            cmd.Dispose()
            ...
            cmd = New SqlCommand("SELECT TOP 1 Id FROM ""ProductImageCrossRef"" WHERE ProductId = @ProductId AND ImageUrl = @ImageUrl ORDER BY Id DESC", conn)
            cmd.Parameters.AddWithValue("@ProductId", ProductId)
            cmd.Parameters.AddWithValue("@ImageUrl", ImageUrl)
            da = New SqlDataAdapter(cmd)
            da.Fill(dt)
    ...
    Public Function InsertIntoProductCross() As Integer
        ...
        Return checkInsert
    End Function
    ''' <summary>Insert a record in the ProductImageCrossRef table from a DataRow</summary>
    ''' <param name="Row">The DataRow to be inserted in the ProductImageCrossRef table</param>
    ''' <returns>'True', if successful</returns>
    ''' <remarks>The DataRow must contain all primary key columns</remarks>
    Public Function Insert(ByRef Row As DataRow) As Boolean
        ...
        Try
            ...
            For Each col in Row.Table.Columns
                ...
                End Select
            Next
            cmd = New SqlCommand("INSERT INTO ""ProductImageCrossRef"" (" & setList & ") VALUES (" & valList & ")", conn)
    ...
    Public Function Update() As Boolean
        Try
            Connect()
            Dim cmd As SqlCommand
            cmd = New SqlCommand("UPDATE ""ProductImageCrossRef"" SET ProductId = @ProductId , ImageUrl = @ImageUrl   WHERE Id = @Id", Conn)
            cmd.Parameters.AddWithValue("@Id", Id)
            cmd.Parameters.AddWithValue("@ProductId", ProductId)
            cmd.Parameters.AddWithValue("@ImageUrl", ImageUrl)
            cmd.ExecuteScalar()
    ...
    Public Function Update(ByRef Row As DataRow) As Boolean
        ...
        Try
            ...
            Next
            cmd = New SqlCommand("UPDATE ""ProductImageCrossRef"" SET " & setList & " WHERE Id = @Id", Conn)
            REM Create the parameters
            For Each col In Row.Table.Columns
                cmd.Parameters.AddWithValue("@" & col.ColumnName, Row(col.ColumnName))
            Next
    ...
    End Function
    ''' <summary></summary>
    ''' <returns>The column names from ProductImageCrossRef table separated by commas</returns>
