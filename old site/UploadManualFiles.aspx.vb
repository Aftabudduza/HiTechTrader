...
#Region "Manual Files Inload Methods"
    Protected Sub btnImport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnImport.Click
        ...
        Try
            If Me.uplProduct.FileName <> "" Then
                ...
                Dim nFile As String = Path.Combine(filePath, "INLOAD_ManualFiles_" & DateTime.Now.ToString("yyyyMMddhhmmss") & ".txt")
                ...
                Dim sMessage As String = icnt & " Manual Files(s) Processed Out Of " & itotcnt & " With " & ierr & " Error(s)"
                lblMsg.Text = sMessage

                Try
                    If sProducts.Length > 0 Then
                        appGlobal.LogDataError("Successful In loading Manual Products Files:" & System.Environment.NewLine & sProducts.ToString())
                    End If
                Catch ex As Exception
                ...
                Try
                    If sFailedProducts.Length > 0 Then
                        appGlobal.LogDataError("Error In loading Manual Products Files:" & System.Environment.NewLine & sFailedProducts.ToString())
                    End If
                Catch ex As Exception
    ...
    Private Function ProcessLine(ByVal sLine As String, ByVal aHeader As ArrayList) As Boolean
        Try
            ...
            For i As Integer = 0 To aHeader.Count - 1
                If aHeader(i).Trim.ToString.Trim <> "" Then
                    If Not Actions.inArray(aSkips, CStr(aHeader(i).Trim).ToUpper) Then
                        If CStr(aHeader(i)).ToUpper = "FILETITLE" Then
                            If aLine(i) <> "" And aLine(i).ToString.Trim.ToUpper <> "NULL" Then
                                sName = aLine(i)
                            End If
                        End If
            ...
            For i As Integer = 0 To aHeader.Count - 1
                If aHeader(i).Trim.ToString.Trim <> "" Then
                    If Not Actions.inArray(aSkips, CStr(aHeader(i).Trim).ToUpper) Then
                        If CStr(aHeader(i)).ToUpper = "PDFFILENAME" Then
                            If aLine(i) <> "" And aLine(i).ToString.Trim.ToUpper <> "NULL" Then
            ...
            If sProductImage.Trim.Length <= 0 Then
                lblMsg.Text = "No File in Inload File"
                Return False
            End If

            If Not GetProductImageCrossRef(sName.Trim, sProductImage.Trim, CInt(sId)) Then
                Return False
            Else
                Return True
            End If
    ...
    Private Function aSkipFields() As ArrayList
        ...
        a.Add("WSMMANUALFILEID")
        a.Add("PDFCOUNT")
        Return a
    End Function

    Private Function GetProductImageCrossRef(ByVal sName As String, ByVal sProductImage As String, ByVal nProductId As Integer) As Boolean
        Try
            ...
            'add ALL new
            Try
                Dim sDel As String = "DELETE FROM ManualProductImageCrossRef WHERE ImageUrl = '" & sProductImage & "' and ProductId=" & nProductId
            ...
            If sProductImage.Length > 0 Then
                If Not String.IsNullOrEmpty(sProductImage) Then
                    Dim sSQL As String = "select id from ManualProductImageCrossRef where [ImageUrl]='" & sProductImage.Trim.Trim & "' and ProductId=" & nProductId
                    sId = SQLData.generic_scalar(sSQL, SQLData.ConnectionString)
                    If sId = "" Then
                        'add it
                        Dim sIns As String = "INSERT INTO ManualProductImageCrossRef ([ProductId], ImageUrl, Name) VALUES('" & nProductId & "','" & sProductImage.Trim & "','" & sName.Trim & "')"
...
#End Region
#Region "Manual Files Upload Methods"
    Public Sub GenerateFile()
        Try

            Dim fileName As String = String.Empty
            Dim str As String = "SELECT p.*  FROM ManualProductImageCrossRef p "
            ...
            If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                wr.Write("ID")
                wr.Write(",")
                wr.Write("WSMManualsID")
                wr.Write(",")
                wr.Write("FileTitle")
                wr.Write(",")
                wr.Write("pdfFileName")
                wr.Write(wr.NewLine)
