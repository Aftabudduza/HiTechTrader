Imports System.IO
Imports System.Data
Partial Class Admin_UploadManufacturerAlternative
    Inherits System.Web.UI.Page
    Public ds As DataSet = New DataSet()
    Dim dt As New DataTable
    Public strConnection As String = appGlobal.CONNECTIONSTRING()
#Region "Manufacturer Inload Methods"
    Protected Sub btnImportService_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnImport.Click
        ...
        Try
            If Me.uplManufacturer.FileName <> "" Then
                'read the file in
                Dim filePath As String = Path.Combine(Request.PhysicalApplicationPath, "Files\Manufacturer\Alternative\")

                If Not Directory.Exists(filePath) Then
                    Directory.CreateDirectory(filePath)
                End If

                Dim nFile As String = Path.Combine(filePath, "INLOAD_ManufacturerAlternative_" & DateTime.Now.ToString("yyyyMMddhhmmss") & ".txt")
    ...
    Private Function ProcessLine(ByVal sLine As String, ByVal aHeader As ArrayList) As Boolean
        Try
            ...
            End If
            For i As Integer = 0 To aHeader.Count - 1
                If CStr(aHeader(i)).ToUpper = "ALSOKNOWNAS" Then
            ...
            If sProdId = "" Then
                ...
                With sb
                    .Append("INSERT INTO ManufacturerAlternative (")
                    For x As Integer = 0 To aHeader.Count - 1
                        If aHeader(x).Trim.ToString.Trim <> "" Then
                            If Not Actions.inArray(aSkips, CStr(aHeader(x).Trim).ToUpper) Then
                                ...
                                If CStr(aHeader(x).Trim).ToUpper = "ALSOKNOWNAS" Then
            ...
            Else
                ...
                With sb
                    .Append("UPDATE ManufacturerAlternative SET ")
                    Dim bFirst As Boolean = True
                    Dim bFirstAdd As Boolean = True
                    Dim bHitPriceBreak As Boolean = False
                    Dim bHasBriefDescription As Boolean = False
...
#Region "Manufacturer Upload Methods"
    Public Sub GenerateFile()
        Try

            Dim fileName As String = String.Empty
            Dim str As String = "SELECT m.[Name], ma.Alternative   FROM Manufacturer m, ManufacturerAlternative ma WHERE m.Id = ma.ManufacturerId ORDER BY m.[Name] asc"
            ...
            If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
                Dim filePath = Path.Combine(Request.PhysicalApplicationPath, "Files\Manufacturer\Alternative\")
                If Not Directory.Exists(filePath) Then
                    Directory.CreateDirectory(filePath)
                End If
                ...
                wr.Write(",")
                wr.Write("Also Known As")
                wr.Write(wr.NewLine)
                For Each dr As DataRow In dt.Rows
                    wr.Write(dr("Name").ToString())
                    wr.Write(",")
                    wr.Write(dr("Alternative").ToString())
                    wr.Write(wr.NewLine)
                Next
                wr.Close()
                Session("ApplicationFile") = filePath
                Response.Redirect("../Files/Manufacturer/Alternative/" & fileName)
            Else
